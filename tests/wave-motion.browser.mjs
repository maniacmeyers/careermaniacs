...
try {
  ...
  await page.waitForFunction(() => document.querySelector('.wave-video').paused)
  assert.equal(await video.isVisible(), false, 'Reduced motion uses the still')
  await page.setViewportSize({ width: 390, height: 844 })
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await page.reload()
  await page.locator('#method').scrollIntoViewIfNeeded()
  assert.equal(await video.getAttribute('src'), null, 'Mobile does not load video')
  assert.equal(await video.isVisible(), false)
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false)
  console.log('Wave motion: lazy load, looping, pause, offscreen pause/resume, reduced motion and mobile passed.')
} finally {
  await browser.close()
}
