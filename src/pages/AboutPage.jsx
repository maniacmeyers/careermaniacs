...
const AboutPage = () => (
  <div className="page about-page">
    ...
    <section className="section-space solid-section"><div className="wrap"><h2 className="headline">The work behind the coaching.</h2><p className="section-deck">Two decades in enterprise sales. Twelve President’s Club wins. Four MVP awards. Two Rookie of the Year awards.</p><div className="career-record">{proofRows.map(row => <article key={row.company}><h3>{row.company}</h3><p className="muted">{row.role}</p><p>{row.result}</p></article>)}</div><p className="record-note">Where I carried a bag. Client stories are on the Testimonials page.</p></div></section>
